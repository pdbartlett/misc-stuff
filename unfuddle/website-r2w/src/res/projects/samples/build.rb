...
class ClrBuildItem < BuildItem
    ...
    def options
        ...
        refs = references
        optList.push("/r:#{refs.join(",")}") unless refs.empty?
        optList.push("#{@path}\\*.#{@tool.srcExtension}")
        optList
    end
    def references
        dependTargets = @dependsOn.collect { |name| BuildItem.getByName(name).target }
    end
end # ClrBuildItem

class ClrApplication < ClrBuildItem
    def initialize(name, tool, path, icon, *dependsOn)
        super("", name, tool, path, *dependsOn)
        @icon = icon
    end
    def options
        optList = super
...
class ClrTestLibrary < ClrClassLibrary
    ...
    def references
        [ "\"#{@@nUnitDir}\\NUnit.Framework.dll\"", *super ]
    end
end # ClrTestLibrary

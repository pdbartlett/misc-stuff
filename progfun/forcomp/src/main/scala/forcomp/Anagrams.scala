...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toLowerCase().groupBy(c => c).map(p => (p._1 -> p._2.length)).toList.sortWith(_._1 < _._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    val str = occurrences.flatMap(p => List.fill(p._2)(p._1)).mkString
    val iseq = for {
      n <- 0 to str.length
      comb <- str.combinations(n)
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val subMap = y.toMap
    x.map(p => (p._1 -> (p._2 - subMap.getOrElse(p._1, 0)))).filter(q => q._2 > 0)
  }
